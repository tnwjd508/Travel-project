...
export function DiagnosisPage() {
  ...
  return <DashboardPageFrame eyebrow="Regional Diagnosis" title={`${district.nameKo} 관광 지표를 진단합니다`} description="관광 지표를 바탕으로 지역 현황과 우선 검토 과제를 확인하세요." icon={BrainCircuit}>
    <DataNotice state={state}/>
    {state.data && <><div className="grid gap-5 xl:grid-cols-2"><section className="rounded-[24px] border border-slate-200 bg-white p-6"><h2 className="text-lg font-bold">지표별 점검</h2><div className="mt-4 space-y-3">{state.data.issues.map(issue => <article key={issue.id} className="rounded-xl border border-slate-100 p-4"><div className="flex items-center justify-between gap-2"><h3 className="text-sm font-bold">{issue.label}</h3><span className={`shrink-0 text-xs font-semibold ${issue.status === 'attention' ? 'text-amber-700' : 'text-slate-500'}`}>{issue.status === 'attention' ? '검토 필요' : issue.status === 'normal' ? '검토 기준 미해당' : '자료 부족'}</span></div><p className="mt-2 text-xs leading-6 text-slate-500">{issue.evidence}</p><p className="mt-2 text-xs leading-6 text-slate-600">{diagnosisCriteria[issue.id]}</p></article>)}</div></section><RegionRadar data={state.data}/></div>
