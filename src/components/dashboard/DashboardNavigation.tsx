...
import { getDistrictDashboardPath } from '@/data/gwangjuDistricts'
...
const dashboardMenuDefinitions = [
  { label: '지역 현황', section: 'overview', icon: House, description: '핵심 현황과 오늘의 브리핑' },
...
export function getDashboardMenuItems(district: DistrictSlug): DashboardMenuItem[] {
  return dashboardMenuDefinitions.map(({ section, ...item }) => ({
    ...item,
    path: getDistrictDashboardPath(district, section),
  }))
}
