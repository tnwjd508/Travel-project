...
export function StrategyTable({ currentPolicy, context }: { currentPolicy?: PolicyId; context: ReturnType<typeof useReviewData> }) {
  ...
  return <Card className="overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-6 sm:px-7"><div><p className="text-[12px] font-bold uppercase tracking-[.15em] text-blue-600">Strategy Compare</p><h3 className="mt-1 text-lg font-bold">진단 연관도로 본 정책 비교</h3></div><div className="flex items-center gap-2 rounded-full bg-blue-50 px-3 py-1.5 text-[12px] font-bold text-blue-600"><BadgeCheck size={14}/>규칙 기반 진단 결과 기준</div></div>
...
function EffectCell({ evidence, frozen }: { evidence: PolicyEvidence | null; frozen: boolean }) {
  ...
  if (evidence.status !== 'available') return <span className="font-semibold text-amber-700">근거 부족<span className="block text-[12px] font-normal text-slate-500">{formatRange(evidence.stat)} · {evidence.stat.n}건</span></span>
  return <span className="font-bold text-emerald-700">외지인 {formatPct(evidence.stat.meanPct)}<span className="block text-[12px] font-normal text-slate-500">{formatRange(evidence.stat)} · {evidence.stat.n}건</span></span>
}
