    ...
    <section aria-labelledby="quick-menu-title">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        {quickMenuItems.map(({ id, title, subtitle, path, icon: Icon }) => (
          <Link
            key={id}
            to={path.replace('/dashboard/gwangju', district.basePath)}
          ...
          >
            ...
            <span className="min-w-0 flex-1"><strong className="block truncate text-xs font-bold text-slate-800">{title}</strong><span className="mt-1 block text-[12px] font-medium text-slate-400">{subtitle}</span></span>
