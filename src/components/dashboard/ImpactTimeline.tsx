...
export function ImpactTimeline({ policy, duration }: { policy: PolicyId; duration: PolicyDuration }) {
  ...
  return <Card className="relative overflow-hidden p-6 sm:p-8"><div className="relative flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] font-bold uppercase tracking-[.15em] text-blue-600">Measurement Plan</p><h3 className="mt-1 text-xl font-bold tracking-tight">{policyLabels[policy]} 성과 측정 계획</h3></div><span className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[10px] font-bold text-slate-500">정책 기간 · {duration}</span></div><div className="relative mt-9 grid gap-4 md:grid-cols-4"><div className="absolute left-[10%] right-[10%] top-6 hidden h-px bg-gradient-to-r from-slate-200 via-blue-300 to-emerald-300 md:block"/>{steps.map((s,i)=><motion.div key={s.title} initial={{opacity:0,y:12}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{delay:i*.1}} className="relative flex items-center gap-3 md:block md:text-center"><div className={`relative z-10 grid h-12 w-12 shrink-0 place-items-center rounded-2xl shadow-lg md:mx-auto ${s.color}`}><s.icon size={19}/></div><div className="md:mt-4"><span className="text-[9px] font-bold uppercase tracking-widest text-slate-400">{s.period}</span><h4 className="mt-0.5 text-sm font-bold text-slate-800">{s.title}</h4><p className="mt-1 text-[10px] leading-4 text-slate-500">{s.sub}</p></div>{i<steps.length-1&&<ChevronRight size={15} className="ml-auto text-slate-300 md:hidden"/>}</motion.div>)}</div><p className="mt-6 text-[11px] leading-5 text-slate-500">예상 수치 대신 측정 계획을 보여줍니다. 실제 시행 결과가 쌓이면 효과 예측 모델의 근거로 사용할 수 있습니다.</p></Card>
}
