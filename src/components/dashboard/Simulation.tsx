...
export function Simulation() {
  ...
  const inputsChanged = scenario != null && (scenario.policy !== selectedPolicy || scenario.duration !== duration)
  ...
  const targetIssues = target && diagnosisState.data
    ...
    : []
  const evidence = scenario ? getEvidence(scenario.policy) : null
  ...
  const extraVisitors = evidence?.status === 'available' && evidence.stat && dailyOutside != null
    ? evidence.stat.ci95Pct.map(pct => Math.round((dailyOutside * pct) / 100 / 100) * 100)
    : null

  return <section>
    <div className="mb-5"><TourismIndicators state={summaryState}/></div>
    <div className="grid gap-5 xl:grid-cols-[.92fr_1.08fr]">
      <Card className="relative overflow-hidden p-6 sm:p-8"><div className="absolute right-0 top-0 h-40 w-40 rounded-full bg-blue-100/50 blur-3xl"/><div className="relative"><div className="mb-6 flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-slate-950 text-white"><FlaskConical size={22}/></div><div><p className="text-[12px] font-bold uppercase tracking-[.16em] text-blue-600">Policy Lab</p><h3 className="text-xl font-bold tracking-tight">정책 조건을 설계하세요</h3></div></div>
        ...
        <p className="mt-3 text-[12px] leading-5 text-slate-500">문화축제는 등록된 과거 전국 축제 사례의 방문 변화를 참고합니다. 기간에 따른 효과 차이는 근거 데이터가 없어 반영하지 않습니다. 예산 입력은 효과를 추정할 근거가 없어 두지 않았습니다.</p>
      </div></Card>
      <Card className={`relative overflow-hidden p-6 transition sm:p-8 ${scenario ? 'border-blue-200' : ''}`}>
        <AnimatePresence mode="wait">{!scenario || !target ? <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex h-full min-h-[430px] flex-col items-center justify-center text-center"><div className="grid h-24 w-24 place-items-center rounded-[32px] bg-slate-50 text-slate-300"><ClipboardList size={42}/></div><h3 className="mt-6 text-xl font-bold text-slate-800">시나리오 검토 결과가 여기에 표시됩니다</h3><p className="mt-2 max-w-xs text-sm leading-6 text-slate-400">정책과 예산, 기간을 선택하면<br/>정책이 겨냥하는 지표의 현재 상태를 보여드립니다.</p></motion.div> : <motion.div key={scenario.createdAt} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .4 }}>
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[12px] font-bold uppercase tracking-[.15em] text-blue-600">Scenario Review</p><h3 className="mt-1.5 text-2xl font-bold tracking-tight">{policyLabels[scenario.policy]}</h3><p className="mt-1 text-xs text-slate-500">{district.nameKo} · {scenario.duration}</p></div>{evidence && <span className={`rounded-full border px-3 py-1.5 text-[12px] font-bold ${evidenceStatusClass[evidence.status]}`}>{evidenceStatusLabels[evidence.status]}</span>}</div>
          {inputsChanged && <p role="status" className="mt-4 rounded-xl bg-blue-50 px-4 py-2.5 text-[12px] text-blue-800">조건이 바뀌었습니다. 새 조건을 반영하려면 다시 검토하세요.</p>}
          ...
          {!evidence && <DataNotice state={evidenceState}/>}
          <div className="mt-2 grid grid-cols-2 gap-2">{outcomeMetrics.map((metric, index) => {
            ...
            return <div key={metric.label} className={`rounded-xl p-3 ${stat && evidence?.status === 'available' ? 'border border-emerald-200 bg-emerald-50/60' : 'border border-dashed border-slate-200'}`}><div className="flex items-center gap-1.5 text-[12px] font-medium text-slate-500"><metric.icon size={13}/>{metric.label}</div>
              {stat && evidence?.status === 'available' ? <><p className="mt-1 text-lg font-bold tracking-tight text-slate-950">{formatPct(stat.meanPct)}</p><p className="text-[12px] leading-4 text-slate-500">외지인 · 축제 기간 · 95% {formatRange(stat)}</p></>
                : stat ? <><p className="mt-1 text-xs font-semibold text-amber-700">근거 부족</p><p className="text-[12px] leading-4 text-slate-500">95% {formatRange(stat)} (0 포함)</p></>
                : <p className="mt-1 text-xs font-semibold text-slate-400">{index === 0 && evidence ? evidenceStatusLabels[evidence.status] : '산출 근거 없음'}</p>}</div>
          })}</div>
          {evidence?.stat && <div className="mt-3 rounded-xl border border-slate-100 p-4 text-[12px] leading-5 text-slate-600">
            <p className="font-bold text-slate-800">근거: {evidence.basis}</p>
            <p className="mt-1">방문자 자료 {evidence.metadata?.visitorsFrom}~{evidence.metadata?.visitorsTo}</p>
            ...
            <p className="mt-1">{evidence.metadata?.method}</p>
            <p className="mt-1 text-slate-500">출처: {evidence.metadata?.sources.join(', ')}</p>
          </div>}
          <p className="mt-4 flex gap-2 rounded-xl bg-slate-50 p-3 text-[12px] leading-5 text-slate-500"><Info size={14} className="mt-0.5 shrink-0"/>{evidence?.status === 'available'
            ? '과거 사례의 평균이며 이 정책의 효과를 보장하지 않습니다. 소비·체류·집중도 변화는 근거 데이터가 없어 표시하지 않습니다.'
