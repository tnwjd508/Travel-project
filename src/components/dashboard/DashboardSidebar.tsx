    ...
    <aside className="fixed bottom-0 left-0 top-[72px] z-30 hidden w-[230px] flex-col overflow-y-auto border-r border-slate-200 bg-white px-4 py-5 lg:flex" aria-label="지역 관광전략 사이드바">
      ...
      <div className="mt-auto space-y-4">
        <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3.5">
          <div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-lg bg-white text-blue-600 shadow-sm"><MapPin size={15} aria-hidden="true" /></span><div><p className="text-[12px] font-medium text-slate-400">현재 지역</p><p className="text-xs font-bold text-slate-800">{district.regionName} {district.nameKo}</p></div></div>
          ...
          <button type="button" onClick={() => navigate('/')} className="mt-2 flex min-h-10 w-full items-center justify-center rounded-xl text-[12px] font-semibold text-slate-400 outline-none transition hover:bg-white hover:text-slate-700 focus-visible:ring-2 focus-visible:ring-blue-500">광역 지역 다시 선택</button>
        </div>

        <div className="grid grid-cols-2 gap-2">
          <button type="button" onClick={onOpenSettings} aria-haspopup="dialog" className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl text-[12px] font-medium text-slate-500 outline-none transition hover:bg-slate-50 hover:text-slate-800 focus-visible:ring-2 focus-visible:ring-blue-500" aria-label="설정"><Settings size={14} />설정</button>
        ...
        <div className="relative overflow-hidden rounded-2xl bg-slate-950 p-4 text-white">
          ...
          <p className="relative mt-2 text-[12px] leading-4 text-slate-300">지역 관광의 길을<br />열어드립니다.</p>
        </div>
      </div>
    </aside>
  )
