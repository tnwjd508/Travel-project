...
type MapAttraction = Pick<Attraction, 'name' | 'category' | 'visitors' | 'accent'> & { x: number; y: number }
...
export function TourismMap() {
  ...
  return <Card className="grid min-h-[470px] overflow-hidden lg:grid-cols-[320px_1fr]">
    ...
    <div className="map-grid relative min-h-[430px] overflow-hidden bg-[#eef5f7]">
      ...
      <div className="absolute left-5 top-5 rounded-xl border border-white/80 bg-white/80 px-3 py-2 shadow-sm backdrop-blur"><p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Data coverage</p><p className="mt-0.5 text-xs font-bold text-slate-700">광주광역시 {activeDistrict.nameKo} <span className="text-blue-600">98.7%</span></p></div>
