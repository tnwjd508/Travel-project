        ...
        <>
          ...
          <motion.aside initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ duration: .22, ease: [0.16, 1, 0.3, 1] }} className="fixed inset-y-0 left-0 z-[60] flex w-[min(88vw,330px)] flex-col overflow-y-auto bg-white p-5 shadow-2xl lg:hidden" aria-label="모바일 대시보드 메뉴">
            <div className="flex items-center justify-between border-b border-slate-100 pb-5">
              <div><div className="flex items-center gap-2"><p className="text-lg font-extrabold tracking-[-.05em]">ON<span className="text-blue-600">:</span>GIL</p><span className="rounded-full bg-blue-50 px-2 py-1 text-[12px] font-bold text-blue-600">AI</span></div><p className="mt-1 text-[12px] text-slate-400">AI 지역 관광전략 수립 플랫폼</p></div>
            ...
            <nav className="mt-5 space-y-1" aria-label="지역 관광전략 카테고리">
            ...
            <div className="mt-auto space-y-3 pt-8">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3.5">
                <div className="flex items-center gap-2"><MapPin size={15} className="text-blue-600" /><div><p className="text-[12px] text-slate-400">현재 지역</p><p className="text-xs font-bold text-slate-800">{district.regionName} {district.nameKo}</p></div></div>
                <button type="button" onClick={() => { onClose(); navigate(district.selectionPath) }} className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-blue-100 bg-white text-[12px] font-semibold text-blue-600 outline-none focus-visible:ring-2 focus-visible:ring-blue-500"><RotateCcw size={14} />자치구 다시 선택</button>
                <button type="button" onClick={() => { onClose(); navigate('/') }} className="mt-2 flex min-h-10 w-full items-center justify-center rounded-xl text-[12px] font-semibold text-slate-400 outline-none focus-visible:ring-2 focus-visible:ring-blue-500">광역 지역 다시 선택</button>
              </div>
              <div className="grid grid-cols-2 gap-2"><button type="button" onClick={onOpenSettings} aria-haspopup="dialog" className="flex min-h-11 items-center justify-center gap-2 rounded-xl text-xs text-slate-500 outline-none hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-blue-500"><Settings size={14} />설정</button></div>
            </div>
          </motion.aside>
        </>
      )}
