...
export function TourismCharts() {
  ...
  return <Card className="p-5 sm:p-7">
    ...
    <div className="mt-5" role="tabpanel" aria-label={tab}>
      ...
      {tab === '방문 추이' && visitors.data && <><p className="mb-2 text-xs text-slate-500">일별 방문 추정치 월 합계 · 파랑: 해당 연도 / 회색: 전년 동월 · 누락 월은 선을 연결하지 않습니다.</p><div className="h-64"><ResponsiveContainer><AreaChart data={series}><CartesianGrid stroke="#e2e8f0" vertical={false}/><XAxis dataKey="month" tick={{ fontSize: 10 }}/><YAxis width={65} tick={{ fontSize: 10 }} tickFormatter={value => `${(value / 10000).toFixed(0)}만`}/><Tooltip formatter={(value: number) => formatValue(value)}/><Area name="전년 동월" type="monotone" dataKey="previous" stroke="#94a3b8" fill="transparent" connectNulls={false}/><Area name="일별 추정치 합계" type="monotone" dataKey="current" stroke="#2563eb" fill="#dbeafe" connectNulls={false}/></AreaChart></ResponsiveContainer></div></>}
      {tab === '연령 지수' && indices.data && <><p className="mb-2 text-xs text-slate-500">연령별 방문 지수 · 실제 인원 또는 연령 구성 비율이 아닙니다.</p><div className="h-64"><ResponsiveContainer><BarChart data={ages}><CartesianGrid vertical={false}/><XAxis dataKey="age" tick={{ fontSize: 10 }}/><YAxis tick={{ fontSize: 10 }}/><Tooltip/><Bar dataKey="value" name="지수" fill="#2563eb" radius={[5, 5, 0, 0]}/></BarChart></ResponsiveContainer></div></>}
      {tab === '콘텐츠 구성' && contents.data && (contents.data.totalCount ? <div className="grid gap-4 sm:grid-cols-2"><div className="h-64"><ResponsiveContainer><PieChart><Pie data={contents.data.typeShare} dataKey="pct" nameKey="category" innerRadius={55} outerRadius={90}>{contents.data.typeShare.map((item, i) => <Cell key={item.category} fill={colors[i % colors.length]}/>)}</Pie><Tooltip formatter={(value: number) => `${value.toFixed(1)}%`}/></PieChart></ResponsiveContainer></div><ul className="space-y-2 py-4">{contents.data.typeShare.map(item => <li key={item.category} className="flex justify-between text-xs"><span>분류 {item.category}</span><span>{item.count}건 · {item.pct.toFixed(1)}%</span></li>)}</ul></div> : <p className="py-8 text-sm text-slate-500">등록된 관광 콘텐츠가 없습니다.</p>)}
...
export function RegionRadar({ data }: { data?: DiagnosisResponse }) {
  ...
  return <Card className="p-5 sm:p-7"><h2 className="text-lg font-bold">관광 지수 6축</h2><p className="mt-2 text-xs text-slate-500">자체 활성화 지수: {formatValue(data?.activationIndex, 2)} · 검토용 단순평균</p>
    {data && data.radar.every(axis => axis.value !== null) ? <div className="mt-3 h-64"><ResponsiveContainer><RadarChart data={data.radar} outerRadius="60%"><PolarGrid/><PolarAngleAxis dataKey="label" tick={{ fontSize: 9 }}/><Radar dataKey="value" stroke="#2563eb" fill="#2563eb" fillOpacity={.16}/><Tooltip/></RadarChart></ResponsiveContainer></div> : <p className="py-8 text-xs text-slate-500">6축 데이터가 모두 제공되면 레이더를 표시합니다.</p>}
    ...
    {showRank && <><DataNotice state={rank}/>{rank.data && <><p className="text-sm font-bold">{rank.data.rank === null ? '순위 자료 부족' : `${rank.data.total}개 관측 시군구 중 ${rank.data.rank}위`}</p><SourceNote data={rank.data}/></>}</>}
  </Card>
}
