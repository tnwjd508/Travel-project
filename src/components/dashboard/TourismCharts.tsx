...
export function TourismCharts() {
  ...
  return <Card className="p-5 sm:p-7">
    ...
    <div className="mt-5 min-h-[240px]" role="tabpanel" aria-label={tab}>
      ...
      {tab === '연령 지수' && indices.data && <><div className="h-48"><ResponsiveContainer><BarChart data={ages}><CartesianGrid vertical={false}/><XAxis dataKey="age" tick={{ fontSize: 10 }}/><YAxis tick={{ fontSize: 10 }} width={55}/><Tooltip formatter={(value: number) => [`${formatValue(value, 2)} (지수)`, '연령별 방문 지수']}/><Bar dataKey="value" name="연령별 방문 지수" fill="#2563eb" radius={[5, 5, 0, 0]}/></BarChart></ResponsiveContainer></div></>}
