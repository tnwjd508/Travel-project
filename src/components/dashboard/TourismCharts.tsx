...
import { ACTIVATION_BASIS, radarMetrics, metricMonth } from '@/data/tourismMetrics'
...
export function TourismCharts() {
  ...
  return <Card className="p-5 sm:p-7">
    ...
    <div className="mt-5" role="tabpanel" aria-label={tab}>
      ...
      {tab === '연령 지수' && indices.data && <><MetricGuide metrics={['age']} month={indices.data.baseYm}/><div className="h-64"><ResponsiveContainer><BarChart data={ages}><CartesianGrid vertical={false}/><XAxis dataKey="age" tick={{ fontSize: 10 }}/><YAxis tick={{ fontSize: 10 }} width={55}/><Tooltip formatter={(value: number) => [`${formatValue(value, 2)} (지수)`, '연령별 방문 지수']}/><Bar dataKey="value" name="연령별 방문 지수" fill="#2563eb" radius={[5, 5, 0, 0]}/></BarChart></ResponsiveContainer></div></>}
...
export function RegionRadar({ data }: { data?: DiagnosisResponse }) {
  ...
  return <Card className="p-5 sm:p-7"><h2 className="text-lg font-bold">관광 지수 6축</h2><p className="mt-2 text-xs text-slate-500">자체 활성화 지수: {formatValue(data?.activationIndex, 2)} · 검토용 단순평균</p><p className="mt-2 text-xs leading-6 text-slate-600">{ACTIVATION_BASIS}</p>{data && <MetricGuide metrics={radarMetrics} month={data.baseYm}/>}
    ...
    {showRank && <><DataNotice state={rank}/>{rank.data && <><p className="text-xs leading-6 text-slate-500">{metricMonth(rank.data.baseYm)} 관광체류강도 지수가 높은 순서입니다. 값이 관측된 시군구만 포함하며 동점은 같은 순위입니다. 전국 전체 지역의 완전한 순위는 아닙니다.</p><p className="text-sm font-bold">{rank.data.rank === null ? '순위 자료 부족' : `${rank.data.total}개 관측 시군구 중 ${rank.data.rank}위`}</p><SourceNote data={rank.data}/></>}</>}
  </Card>
}
