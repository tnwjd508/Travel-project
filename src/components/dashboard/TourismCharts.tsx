...
const tabs = ['방문 추이', '연령 지수', '콘텐츠 구성', '함께 찾는 관광지'] as const
...
export function TourismCharts() {
  ...
  const related = useDistrictResource('related', { district: district.slug }, tab === '함께 찾는 관광지')
  ...
  const hasVisitorData = (visitors.data?.series ?? []).some(point => point.total != null)
    || (visitors.data?.previousYear ?? []).some(point => point.total != null)
  ...
  return <Card className="p-5 sm:p-7">
    ...
    <div className="mt-5 min-h-[240px]" role="tabpanel" aria-label={tab}>
      ...
      {tab === '함께 찾는 관광지' && related.data && <div className="space-y-3">
        <p className="text-xs leading-5 text-slate-500">각 관광지를 방문한 사람들이 함께 찾은 장소입니다. 관광지를 누르면 상위 5곳을 볼 수 있습니다. 건수는 연결된 장소 수이며 방문객 수가 아닙니다.</p>
        <p className="text-sm">상위 3개 관광지의 연관 건수 비중: <strong>{formatValue(related.data.top3Share, 2)}{related.data.top3Share !== null && '%'}</strong></p>
        {related.data.hubs.length ? related.data.hubs.map(hub => (
          <div key={hub.tAtsCd} className="overflow-hidden rounded-xl bg-slate-50">
            <button type="button" onClick={() => setOpenHub(openHub === hub.tAtsCd ? null : hub.tAtsCd)} aria-expanded={openHub === hub.tAtsCd} className="flex min-h-11 w-full items-center justify-between gap-3 px-3 text-xs">
              <span className="flex items-center gap-1.5 font-semibold text-slate-700"><ChevronRight size={13} className={`transition ${openHub === hub.tAtsCd ? 'rotate-90' : ''}`}/>{hub.name}</span>
              <span className="text-slate-600">{hub.relatedCount}건 · {hub.share.toFixed(1)}%</span>
            </button>
            {openHub === hub.tAtsCd && <ol className="space-y-1 px-3 pb-3 pt-1">
              {hub.top.map(item => <li key={`${item.rank}-${item.name}`} className="flex items-center gap-2 rounded-lg bg-white px-3 py-2 text-[12px]">
                <span className="grid h-5 w-5 shrink-0 place-items-center rounded-md bg-slate-100 font-bold text-slate-500">{item.rank}</span>
                <span className="flex-1 font-semibold text-slate-700">{item.name}</span>
                <span className="text-slate-500">{[item.category, item.district].filter(Boolean).join(' · ')}</span>
              </li>)}
              {!hub.top.length && <li className="text-[12px] text-slate-500">연관 장소 정보가 없습니다.</li>}
            </ol>}
          </div>
        )) : <p className="text-xs text-slate-500">함께 찾는 관광지 자료가 없습니다.</p>}
      </div>}
    </div>
    {active.data && (tab !== '방문 추이' || hasVisitorData) && <SourceNote data={active.data}/>}
  </Card>
}
