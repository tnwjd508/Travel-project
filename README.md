...
공모전 제출 화면은 별도 로그인을 사용하지 않는다. 대한민국 지도에서 시군구를 선택하면 정규 지자체 ID가 포함된 대시보드 주소로 이동하며, 월간 브리핑은 `region_id + district_id + month` 단위로 조회·저장된다. 기존 기관 로그인·시나리오 공동 저장 코드는 향후 운영 확장용으로 남아 있지만 공모전 화면에서는 노출하거나 호출하지 않는다.
...
| 정책 시뮬레이션 상태 | 구현 완료 | 입력 조건 저장과 기관별 검토 스냅샷 저장·재조회. 서버 연결 설정 필요 |
...
| 관광 API 영속 캐시 | 적용 준비 | 모든 한국관광공사 API 페이지를 Supabase에 공유하고 상류 장애 시 저장 응답 사용. 관광객 추이는 월별 집계도 별도 보관. 후속 마이그레이션 적용 필요 |
| 실데이터 연동 | 구현 완료 | 개요 KPI·방문 차트·지수·콘텐츠·지도 좌표·축제·진단·순위·보고서 연결 |
| 정책 효과 모델 | 과거 근거 참조 | 등록한 축제 분석 통계를 참조. 예산·기간별 미래 효과는 산출하지 않음 |
...
| 전국 상세 지도·정책 화면 | 로컬 통합 | 전국 라우트 지원. 정책 근거의 지역 적용 범위는 별도로 판별 |
...
4. 저장된 기준선과 정책별 목표 지표를 비교하고 과거 분석 근거 확인
...
현재 권고 구조는 [2026-09-20 데이터베이스 재설계](docs/supabase-database-design.md)를 참고하세요. 월간 브리핑·기관별 시나리오에 과거 축제 근거 버전과 검토 스냅샷을 연결합니다. [이전 통합 기록](docs/supabase-integrated-design.md)은 로컬 구현 이력이며, 새 검토 API·UI와 등록 CLI는 로컬 구현했습니다. 실제 DB의 후속 SQL 적용·자료 등록·서버 연결 절차는 [FastAPI 구현 및 연결 안내](docs/fastapi-implementation-plan.md)를 따르세요.
