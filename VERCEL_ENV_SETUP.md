...
| FastAPI | `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY` | 같은 프로젝트의 DB 주소와 브라우저 로그인용 공개 키 |
| FastAPI | `SUPABASE_SECRET_KEY` 또는 `SUPABASE_SERVICE_ROLE_KEY` | 브리핑 저장·기관 시나리오 서버 RPC용 비밀키 |
...
`/api/monthly-briefing`도 같은 FastAPI와 서버 토큰을 사용한다. TOUR/GEMINI/Supabase 키를 Vercel에 중복 설정할 필요는 없다. `vercel.json`의 월간 함수 300초 설정을 유지하며 Node 분석 240초, DB 임대 280초, FastAPI 285초, 전달 함수·UI 290초로 제한한다. 월간 브리핑을 실행할 FastAPI 호스트에도 Node.js 및 `npm ci --omit=dev`로 설치한 운영 의존성이 필요하다. 워커가 사용하는 TypeScript 원본 파일을 배포에 포함한다.

2026-09-20 로컬 통합 기준 구현 상태와 후속 API는 [FastAPI 구현 방향](docs/fastapi-implementation-plan.md)을 참고한다.
