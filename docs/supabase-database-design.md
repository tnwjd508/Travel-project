...
현재 `policyEvidence(policy)`는 지역을 받지 않는다. 전국 대시보드에 합칠 때 모든 시군구에 metroGu 결과를 자동 적용하면 안 된다. 제안 SQL은 분석 코드 `is_metro_gu`의 현재 규칙을 `festival-reference-v1`로 고정하고 군/범위 밖을 구분한다. 향후 행정구역 개편·범위 변경은 새 선택 규칙 버전으로 처리한다. 야간 통계도 해당 지역 자체의 예측이라고 표현하지 않는다.
